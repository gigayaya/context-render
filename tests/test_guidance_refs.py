...
Dry-run findings (2026-07-19, SPIKES W5 candidates) are frozen here as regression tests:
finding 1 (bare-basename references), finding 4 (slash-idiom stale pollution).
"""
...
def test_runtime_artifact_not_edge_not_stale(index):
    # spec §8:runtime 產物依真實檔案系統解析——存在但被 SKIP 的路徑既非邊也非 stale
...
# ---- W5 #28 迴歸(2026-07-19 跨專案 dry-run)----
...
def test_index_skips_conventional_dirs(tmp_path):
    ...
    (repo / ".DS_Store").write_text("x", encoding="utf-8")   # W5 #28: dotfiles skipped
