...
def test_wildcard_git_valueless_and_value_global_flags_skipped():
    assert extract_mutations("git --no-pager checkout main", CWD)[1] == ["git checkout"]
    assert extract_mutations("git -c core.x=1 pull", CWD)[1] == ["git pull"]


def test_git_flags_only_segment_yields_nothing():
    assert extract_mutations("git -C /repo", CWD) == ([], [])


def test_git_stash_without_pop_or_apply_is_not_wildcard():
    assert extract_mutations("git stash", CWD) == ([], [])
    assert extract_mutations("git stash list", CWD) == ([], [])
