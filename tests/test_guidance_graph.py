"""Reachability closure: reference edges + mechanic edges (guidance-reachability spec §2).

Toggle defaults are pre-W5 leanings (spec §7); the interface is verdict-stable — only the
default constants change at the W5 gate (W4 precedent).
"""
...
def test_w5_locked_defaults():
    # W5 verdicts #24–#26 (2026-07-19, frozen): overturning one later is a
    # change-management event — this test is the tripwire.
    from context_render.guidance.graph import (
        ALL_MD_DEFAULT, DIR_CHILDREN_DEFAULT, FENCED_DEFAULT)
...
def test_default_build_includes_fenced_routing(mini_repo, index):
    # W5 #24: tree-diagram routing inside fenced blocks counts by default
    r = _reach(mini_repo, index)
