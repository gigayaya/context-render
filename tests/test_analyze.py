"""Self-derivation aggregation/rendering: the report SELF-DERIVATION block
(window scope), the session block, facts backfill/coverage, and --emit-prompt."""
...
from context_render.report.aggregate import aggregate_session, aggregate_window
...
# ---- report (window) SELF-DERIVATION block ----

def _window_agg(fake_repo, fake_projects):
    _prep(fake_repo)
    _add_session(fake_projects, fake_repo, "33333333-aaaa-bbbb-cccc-000000000003",
                 search_lines(fake_repo))
    scan_repo(fake_repo, Config(), projects_dir=fake_projects)
    ...
    store = _store(fake_repo)
    try:
        return aggregate_window(store, comps, Config(), since_iso=None,
                                since_label="all history")
    finally:
        store.close()


def test_window_aggregate_carries_self_derivation(fake_repo, fake_projects):
    """`report` owns the window-scope self-derivation rows (formerly `analyze`): same
    grouping as aggregate_analyze, attached to the window aggregate."""
    agg = _window_agg(fake_repo, fake_projects)
    rows = agg["self_derivation"]
    keys = [r["key"] for r in rows]
    assert "retrypolicy" in keys and "repo layout" in keys
    s = agg["self_derivation_summary"]
    assert s["tokens"] == sum(r["tokens"] for r in rows)
    assert s["pct"] is not None
    assert s["facts_sessions"] == 2 and agg["window"]["session_count"] == 2


def test_window_lines_selfderive_block(fake_repo, fake_projects):
    agg = _window_agg(fake_repo, fake_projects)
    body = "\n".join(window_lines(agg, Config(), full=False))
    assert "SELF-DERIVATION" in body
    assert "facts: 2 of 2 sessions" in body
    assert "questions the harness didn't answer" in body
    assert "sessions" in body.split("what the agent was after")[1].splitlines()[0]  # window column set
    assert "repo layout" in body
    # md and terminal share the line builders
    md = render_md(agg, Config())
    for line in window_lines(agg, Config(), full=True):
        assert line.rstrip() in md


def test_window_selfderive_block_top10_and_md_full():
    rows = [{"kind": "keyword", "label": f"needle_{i}", "key": f"needle_{i}",
             "method": "grep", "sessions": 1, "times": 1, "tokens": 100 - i,
             "occupancy": None, "heuristic": False, "story": {}} for i in range(12)]
    agg = {"window": {"label": "all history", "session_count": 3, "project": "p"},
           "components": [], "daily": [], "static": {"total_tokens": 0, "cost_usd": None},
           "cost": {"total_usd": None}, "billing": "subscription", "hints": [], "warnings": [],
           "self_derivation": rows,
           "self_derivation_summary": {"tokens": 1000, "pct": None, "facts_sessions": 3}}
    short = "\n".join(window_lines(agg, Config(), full=False))
    assert "SELF-DERIVATION — top 10 of 12" in short and "--md for all" in short
    assert short.count("needle_") == 10
    full = "\n".join(window_lines(agg, Config(), full=True))
    assert "SELF-DERIVATION — 12 item(s)" in full
    assert full.count("needle_") == 12


def test_window_selfderive_block_empty_says_so():
    agg = {"window": {"label": "all history", "session_count": 1, "project": "p"},
           "components": [], "daily": [], "static": {"total_tokens": 0, "cost_usd": None},
           "cost": {"total_usd": None}, "billing": "subscription", "hints": [], "warnings": [],
           "self_derivation": [],
           "self_derivation_summary": {"tokens": 0, "pct": None, "facts_sessions": 1}}
    body = "\n".join(window_lines(agg, Config(), full=False))
    assert "no self-derivation detected in window" in body
...
def test_stale_extractor_version_triggers_reingest(fake_repo, fake_projects):
    """A DB built by an older extractor (marker without the extractor field = v1)
    ...
    del ev["extractor"]  # simulate a v1-build marker
    store.conn.execute(
...
def test_analyze_counts_uncovered_sessions(fake_repo, fake_projects):
    ...
    # the same coverage warning reaches the report body via aggregate_window
    comps = [c for c in scan_components(fake_repo) if c.provenance == "local"]
    store = _store(fake_repo)
    try:
        wagg = aggregate_window(store, comps, Config(), since_iso=None, since_label="all history")
    finally:
        store.close()
    assert wagg["self_derivation_summary"]["facts_sessions"] == 0
    assert any("no session in this window has extracted facts" in w for w in wagg["warnings"])
...
def test_report_selfderive_cli(fake_repo, fake_projects, monkeypatch):
    ...
    r_bad = runner.invoke(app, ["report", "--emit-prompt", "no-such-key"])
    assert r_bad.exit_code == 3


def test_analyze_command_removed():
    """`analyze` is not a command: its table is `report`'s SELF-DERIVATION block. The old
    entry is gone, and help no longer lists it as a command."""
    assert runner.invoke(app, ["analyze"]).exit_code == 2
    help_out = runner.invoke(app, ["help"]).output
    assert "\n  analyze" not in help_out
    assert "--emit-prompt" in help_out
