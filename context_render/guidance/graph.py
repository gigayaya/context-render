"""Reachability closure over guidance.
...
Toggle defaults below are locked constants, validated across 5 repos on
2026-07-19; the interface never changes with them.
...
# Locked defaults (2026-07-19, n=5 repos; change only with fresh corpus evidence):
FENCED_DEFAULT = True        # fenced refs ARE routing (tree-diagram convention);
#                              resolution itself filters the pollution (hypothetical
#                              example names never resolve → stale channel, not edges)
ALL_MD_DEFAULT = True        # decisive — guidelines/knowledge-base style projects
#                              go 0/21 → 16/21 py on this toggle alone
DIR_CHILDREN_DEFAULT = True  # real one-ls-away coverage; `ls` provenance stays
...
def build_reach(root: Path, index: FileIndex, *,
    ...
    # direct child files per directory, in sorted order — built once instead of scanning
    # (and re-sorting) the whole file set for every directory node that gets popped
    children: dict[str, list[str]] = {}
    if dir_children:
        for f in sorted(index.files):
            children.setdefault(posixpath.dirname(f), []).append(f)
    ...
    while heap:
        ...
        if dir_children and node in index.dirs:
            for f in children.get(node, ()):
