...
Stale detection is strict: an unresolvable candidate is stale only if
    ...
    def __init__(self, root: Path):
        ...
        for dirpath, dirnames, filenames in os.walk(self.root):
            ...
            for f in sorted(filenames):
                if f.startswith("."):  # dotfiles (.DS_Store, .gitignore, submodule
...
def _candidates(text: str):
    ...
    include/exclude it via the locked toggle without re-extraction."""
...
def _prose_tokens(text: str):
    for piece in _SPLIT_RE.split(text):
        tok = _TRAIL_PUNCT_RE.sub("", piece.strip("`*\"'<>[]").rstrip("/"))
...
def _clean(raw: str) -> str | None:
    ...
    if (not raw or " " in raw or raw in (".", "..")
            # leading "/": absolute paths, slash-commands (/docs-drift), XML tags
...
def _resolve(raw: str, base_rel: str, index: FileIndex):
    ...
    that exist on disk but are excluded from the index (runtime artifacts)."""
    if any(ch in raw for ch in _GLOB_CHARS):
        return _resolve_glob(raw, base_rel, index)
    # one normalization per base; the same pair is reused by the on-disk check below
    norms = [n for n in (_norm(base_rel, raw), _norm(".", raw)) if n]
    for norm in norms:
        if norm in index.files or norm in index.dirs:
            return [(norm, "exact")]
    if ("/" not in raw and raw.endswith(PATHISH_EXT)  # layer 3: unique basename
            and index.basename_exists(raw)):
...
def _is_stale(raw: str, base_rel: str, index: FileIndex) -> bool:
    if not _pathish(raw):
        return False
    if "<" in raw or ">" in raw:  # template placeholders (lib/<domain>/x.py)
        return False
    first = raw.split("/")[0]
