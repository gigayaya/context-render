...
| `guidance/` | Static guidance-graph primitives (format-neutral reference extraction, closure, symbols) consumed by `mapdev/` | Working under it (own CLAUDE.md) |
| `mapdev/` | `ctxr map` (merged report: carriers, structure, dead routes, coverage with facts join) and `map init`: paper-convention metrics + skeleton generation over `guidance/` primitives | Working under it (own CLAUDE.md) |
