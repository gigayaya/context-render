...
| `coverage.py` | Coverage aggregation + analyze-facts join (chain_read attribution once per key-file pair; raws are repo-relative since facts extractor v3 and match the FileIndex directly; store=None degrades gracefully); unreachable sorted by observed cost — gauge, not lint | Changing coverage output or the join rules |
...
| `render_term.py` | Terminal renderer; `session_lines`/`window_lines`/`analyze_lines`/`coverage_lines` shared with md; fixed `WRAP_WIDTH = 96` (not live terminal width) | Adding or altering report lines/sections |
