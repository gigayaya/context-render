...
from ..attributor.facts import MAPPING_KEY, Fact
...
def aggregate_rows(fact_rows: list) -> list[dict]:
    ...
    for f in fact_rows:
        g = groups.setdefault(
            ...
            {
                "key": f["key"],
                "kind": "action" if f["key"] == MAPPING_KEY else "keyword",
                "sessions": set(),
                "times": 0,
                "tokens": 0,
                "occupancy": None,
    ...
    for g in groups.values():
        ...
        if variants >= 2:  # the agent was guessing project vocabulary (§4.1 rule 5)
            method += f", {variants} variants tried"
        if g["chain"]:
            method += "+read chains" if method else "read chains"
        if g["heuristic"]:
            method += " ~"
...
def aggregate_analyze(store: Store, config: Config, since_iso: str | None,
    ...
    if not sessions:
        ...
        warnings.append(
            "⚠ no session in this window has extracted facts — run `context-render sync` "
...
def emit_prompt_text(agg: dict, fact_rows: list[dict], row: dict) -> str:
    ...
    w = agg["window"]
    lines = [
        "Scaffolding-gap evidence bundle (context-render analyze --emit-prompt; "
