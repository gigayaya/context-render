"""Self-derivation aggregation: the `report` SELF-DERIVATION block (window scope)
and the session report's block share this grouping — the only differences are scope
(window vs one session) and which columns render.
...
from ..attributor.facts import CODE_KEY, MAPPING_KEY
...
def aggregate_rows(fact_rows: list) -> list[dict]:
    ...
    for f in fact_rows:
        g = groups.setdefault(
            f["key"],
            {
                ...
                "chain": False,
                "heuristic": False,
                "story_searches": Counter(),  # (tool, raw) -> n; a repo-layout probe has raw ""
        ...
        if f["kind"] == "chain_read":
            ...
            if f["key"] == CODE_KEY:
                g["raws"][f["raw"]] += 1  # the method column shows probe shapes, not _mapping_head
    ...
    for g in groups.values():
        if g["kind"] == "action" and g["key"] == CODE_KEY:
            ...
            variants = len(g["raws"])
            method = ", ".join(g["heads"])
            if variants >= 2:  # the agent was guessing project vocabulary
...
def aggregate_analyze(store: Store, config: Config, since_iso: str | None,
    ...
    """(window self-derivation aggregate, raw fact rows) — the fact rows feed --emit-prompt."""
    ...
    if not sessions:
        warnings.append(
            "⚠ no ingested sessions in window — nothing to aggregate; "
...
def emit_prompt_text(agg: dict, fact_rows: list[dict], row: dict) -> str:
    ...
    w = agg["window"]
    lines = [
        ("Scaffolding-gap evidence bundle (ctxr report --emit-prompt; "
