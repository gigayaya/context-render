"""Bash-mediated event heuristics.
...
# POSIX grep and rg disagree on short flags — rg's -r (--replace) takes a value, POSIX
# grep's -r (recursive) is flagless — so the sets are kept separate and callers pick by
# command head; using the rg set for grep would eat the real pattern of `grep -r PAT file`.
GREP_VALUE_FLAGS = {
    "-A", "-B", "-C", "-m", "-e", "-f", "-d", "-D",
...
GREP_RG_VALUE_FLAGS = GREP_VALUE_FLAGS | {
    # rg-only
    "-g", "-t", "-T", "-E", "-M", "-j", "-r",
...
def split_segments_seps(command: str) -> list[tuple[str, list[str]]]:
    ...
    """
    # fresh lists per call: callers may mutate their copy without touching the cache
    return [(sep, list(argv)) for sep, argv in _split_segments_cached(command)]


@lru_cache(maxsize=4096)
def _split_segments_cached(command: str) -> tuple[tuple[str, tuple[str, ...]], ...]:
    """Pure tokenization of `command` (see split_segments_seps). One Bash event is
    tokenized by several independent extractors (git-commit detection, read paths,
    facts, mutations, stale reads), so the shlex pass is memoized on the command text."""
...
def git_subcmd(argv: list[str]) -> list[str] | None:
    """[subcommand, following args...] of a git segment, else None (non-git head, or a
    segment that is nothing but global flags). Global flags are skipped, value flags
    together with their value — the one reading shared by commit detection here,
    `git grep` facts (facts.py) and wildcard mutations (bash_mutations.py)."""
...
def _is_git_commit_segment(argv: list[str]) -> bool:
    sub = git_subcmd(argv)
    return sub is not None and sub[0] == "commit"


def detect_git_commit(command: str, result_text: str | None) -> bool:
    """In-segment git commit candidate + tool_result double confirmation."""
...
def extract_read_paths(command: str, cwd: str | None) -> list[str]:
    ...
    for seg in split_segments(command):
        ...
        value_flags = GREP_RG_VALUE_FLAGS if cmd == "rg" else GREP_VALUE_FLAGS
        ...
        while i < len(seg):
            tok = seg[i]
            if tok.startswith("-"):
                if is_grep and tok in value_flags:
