...
bash search commands — both channels), structure-mapping actions (`repo layout`), and
chain reads (a Read sandwiched between searches whose path came out of the
previous search's results).

Extraction happens at ingest: transcripts expire (~30 days) and these event-level signals
exist nowhere else once they do. Decontamination rules are locked against real dry-run samples —
...
# sync re-extracts sessions whose marker carries an older number (missing field = 1).
...
class FactExtraction:
    ...
    # denominator for the SELF-DERIVATION thesis line
    tool_output_tokens_est: int = 0


def fact_rows(facts: list[Fact], session_id: str) -> list[dict]:
    """Facts in the shape of `facts` table rows — the one owner of that shape, used by the
    ingest path (pipeline.build_fact_rows) and the live session report alike."""
    return [
        {
...
def _grep_argv(argv: list[str]) -> tuple[str, list[str]] | None:
    ...
    if head in GREP_FAMILY:
        return head, argv[1:]
    if head == "git":  # cheap gate: most segments are neither grep nor git
        sub = git_subcmd(argv)
        if sub is not None and sub[0] == "grep":
            return "git grep", sub[1:]
    return None


def _grep_pattern(head: str, args: list[str], piped: bool) -> str | None:
    """Pattern of a grep-family segment, after the decontamination gates:
...
def _scan_find(args: list[str]) -> _FindScan:
    ...
    while i < len(args):
        ...
        i += 1
    if out.has_prune:
        # a -prune segment is exclusion machinery: its -name values are the
...
def _window(ev: Event) -> tuple[bool, str | None]:
    """Context-window identity (window separation): main chain vs each subagent."""
...
def extract_facts(parsed: ParsedSession,
    ...
    ext.tool_output_tokens_est = sum(
        estimate_tokens(ev.text or "") for ev in parsed.events
    ...
    for ev in parsed.events:
        ...
        if tool == "Read":
            ...
            # chain_read candidate (both conditions): path came out of the previous
...
def _attach_occupancy(facts: list[Fact], events: list[Event]) -> None:
    ...
    next compaction boundary — per context window (window separation: a sidechain fact
