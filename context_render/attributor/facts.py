...
# grep/ag flags whose value arrives as the next token. Deliberately NOT the shared
# GREP_RG_VALUE_FLAGS: that set includes rg-only flags like -r (--replace) that for
# POSIX grep mean something flagless (-r = recursive) — consuming their "value" would
# eat the real pattern.
GREP_VALUE_FLAGS = {
    "-A", "-B", "-C", "-m", "-e", "-f", "-d", "-D",
...
class Fact:
    ...
    raw: str  # original pattern (search) / command segment (mapping) / read path (chain_read)
...
class _CallFacts:
    ...
    searches: list[tuple[str, str, str]] = field(default_factory=list)  # (tool, raw pattern, key)
    mappings: list[tuple[str, str]] = field(default_factory=list)  # (raw segment, confidence)
        ...
        if grep is not None:
            ...
            if pattern:
                for part in split_alternation(pattern):
                    out.searches.append((label, _part_raw(part), canonical_key(part)))
            continue
        if head == "fd":
            pattern = _fd_pattern(argv[1:])
            if pattern:
                for part in split_alternation(pattern):
                    out.searches.append(("fd", _part_raw(part), canonical_key(part)))
            continue
        if head == "find":
            scan = _scan_find(argv[1:])
            for name in scan.names:
                for part in split_alternation(name):
                    out.searches.append(("find", _part_raw(part), canonical_key(part)))
            if scan.has_type_d and not scan.names:
                out.mappings.append((_seg_raw(argv), "exact"))
            continue
        if head == "tree":
            out.mappings.append((_seg_raw(argv), "exact"))
            continue
...
def extract_facts(parsed: ParsedSession) -> FactExtraction:
    ext = FactExtraction()
    ...
    for ev in parsed.events:
        ...
        if tool in ("Grep", "Glob"):
            ...
            for part in parts:
                add(Fact(idx=ev.idx, kind="search", key=canonical_key(part),
                         raw=_part_raw(part), tool=tool, tokens_est=per,
                         sidechain=ev.is_sidechain, agent=ev.agent))
            ...
            if (isinstance(target, str) and prev is not None and result is not None
                ...
                pending_reads.setdefault(win, []).append(
                    Fact(idx=ev.idx, kind="chain_read", key=prev[2],
                         raw=clean(target)[:_RAW_CAP], tool="Read",
        ...
        for label, raw, key in call.searches:
            add(Fact(idx=ev.idx, kind="search", key=key, raw=raw, tool=label,
                     tokens_est=per, sidechain=ev.is_sidechain, agent=ev.agent))
        for raw, conf in call.mappings:
            add(Fact(idx=ev.idx, kind="mapping", key=MAPPING_KEY, raw=raw, tool="Bash",
