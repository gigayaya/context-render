"""Stale-copy window extraction (upgrade-plan §2; design specs/2026-08-01-stale-gauge-design.md).

Walks a parsed session once and, per context window (main chain and each
subagent, W2 separation) per repo file, tracks whether a copy read into the
window was later overturned by a mutation without being re-read — the
"read → mutated → never re-read" accident site (#43). Gauge, not grader:
...
_Window = tuple[bool, str | None]  # (is_sidechain, agent) — W2 window identity
...
def extract_stale(parsed: ParsedSession,
    ...
    def record_mutation(mut_win: _Window, rel: str | None, idx: int, tool: str,
        ...
        for key in list(in_context):
            ...
            if copy.read_conf == "heuristic" or win != mut_win or wildcard:
                wconf = "heuristic"
            if key not in stale_open:  # first trigger holds (design §2.2 item 3)
    ...
    for ev in parsed.events:
        ...
        if ev.kind == "compaction":
            ...
            # compacted, reset IN-CONTEXT to CLEAN (design §2.2 item 4)
